...
#'
#' @return A list will be return, including mag, Rrup, lnY, specT, iflag, phi1, tau1, sigma.
#'
#' @examples
#' Arroyo2010(6, 20, 0)
#' Arroyo2010(7, 10, 0)
...
Arroyo2010 <- function(Mag, Rrup, Prd) {
  #Arroyo2010 ( mag, rRup, lnSa, specT, iflag, phi1, tau1, sigma)
  retvals <- .Fortran("Arroyo2010", mag=as.single(Mag), rRup=as.single(Rrup), lnSa=as.single(0.1),
                      specT=as.single(Prd), iflag=as.integer(1), phi1=as.single(0.1),
                      tau1=as.single(0.1), sigma=as.single(0.1))
  names(retvals) <- c("mag", "Rrup", "lnY", "specT", "iflag",  "phi", "tau","sigma")
  return(retvals)
}
