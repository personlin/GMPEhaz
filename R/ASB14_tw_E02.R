...
#'
#' \code{ASB14.tw.E02} returns the ground-motion prediction with it sigma of Akkar et al.(2014) GMPE
#' adjusted to Taiwan.
#'
...
#'
#' @export
ASB14.tw.E02 <- function(Mag, Rjb, Prd=0, ftype=0, Vs30=760){
  #ASB_2013 ( mag, Rbjf, specT,
  #                     period2, lnY, sigma, iflag, ftype, Vs, phiT, tauT )
  if (Prd != 0 & (Prd < 0.01 | Prd > 4)) {
    stop("Period out of range! \n\n")
  }
  retvals <- .Fortran("S04_ASB14_TW_E02", mag=as.single(Mag), Rbjf=as.single(Rjb), specT=as.single(Prd),
